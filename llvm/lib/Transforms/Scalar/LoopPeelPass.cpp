...
static cl::list<std::string> PeelHeaders("peel-header", cl::ZeroOrMore);
...
bool LoopPeel::runOnLoop(Loop *L, LPPassManager &LPM) {
  ...
  if(PeelHeaders.size() > 0) {

    std::string loopName = L->getHeader()->getName();
    std::vector<std::string>::iterator found = find(PeelHeaders.begin(), PeelHeaders.end(), loopName);
    if(found == PeelHeaders.end()) {
      DEBUG(dbgs() << "Won't peel loop " << loopName << " because we've been given specific targets, and this isn't one\n");
      return false;
    }
