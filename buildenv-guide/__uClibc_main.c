...
// Grim hack: an alt version of uclibc_main which doesn't set __environ or do much with its args, for use in specialisation.
// It also takes a main() with the more usual i32, i8** type since that allows us to avoid the type pun involved in the usual
// main function which we get away with thanks to the calling convention.

void __uClibc_main_spec(int (*main)(int, char **), int argc,
			char **argv, char **environ_in, void (*app_init)(void), void (*app_fini)(void),
...

{
    ...
    /* Use a supplied environ (violates ABI), probably breaks setenv etc. */
    __environ = environ_in;
...
void __uClibc_main(int (*main)(int, char **, char **), int argc,
		    ...
		    void (*rtld_fini)(void), void *stack_end attribute_unused)
{
  char** __loc_environ = &argv[argc + 1];
  /* If the first thing after argv is the arguments
   * then the environment is empty. */
  if ((char *) __loc_environ == *argv) {
    /* Make __environ point to the NULL at argv[argc] */
    __loc_environ = &argv[argc];
  }

  __uClibc_main_spec(main, argc, argv, __loc_environ, app_init, app_fini, rtld_fini, stack_end);
}
