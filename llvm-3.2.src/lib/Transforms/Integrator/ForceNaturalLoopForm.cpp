...
namespace {
  ...
  public:
    ...
    bool makeDominateFrom(BasicBlock* BB, BasicBlock* Header, BasicBlock* Target, DenseSet<BasicBlock*>& ReachesTarget, DenseSet<BasicBlock*>& Visited);
...
bool ForceNaturalLoopFormPass::makeDominateFrom(BasicBlock* BB, BasicBlock* Header, BasicBlock* Target, DenseSet<BasicBlock*>& ReachesTarget, DenseSet<BasicBlock*>& Visited) {
  ...
  for(succ_iterator SI = succ_begin(BB), SE = succ_end(BB); SI != SE; ++SI) {
    ...
    BasicBlock* SBB = *SI;
    if(DT->dominates(Header, SBB)) {
      
      if(makeDominateFrom(SBB, Header, Target, ReachesTarget, Visited))
	return true;
      
    }
    else if(ReachesTarget.count(SBB)) {

      errs() << F->getName() << ": In-loop block " << BB->getName() << " has out-of-loop successor " << SBB->getName() << " that can reach target " << Target->getName() << "; rerouting...\n";

      // This block must be dominated by Header if Target will be.
      // Reroute all incoming branches from non-loop blocks.
	...
	for(SmallVector<BasicBlock*, 4>::iterator it = RerouteBBs.begin(), itend = RerouteBBs.end();
	  ...
	  BasicBlock* PBB = *it;
	  BasicBlock* Temp = BasicBlock::Create(BB->getContext(), "nlf.route", BB->getParent(), SBB);
	...
	OldPN->addIncoming(NewPN, Header);
      ...
      // Finally add a switch case: when entered with nlf.predidx == SBBID, branch to SBB.
...
bool ForceNaturalLoopFormPass::makeDominate(BasicBlock* Header, BasicBlock* Target) {
  ...
  DenseSet<BasicBlock*> ReachesTarget;
  gatherPreds(Target, ReachesTarget);

  DenseSet<BasicBlock*> Visited;

  bool ret;

  if((ret = makeDominateFrom(Header, Header, Target, ReachesTarget, Visited))) {
...
bool ForceNaturalLoopFormPass::forceOneBackedge() {
  ...
  for(Function::iterator it = F->begin(), itend = F->end(); it != itend; ++it) {
	...
	if((!L) || L->getHeader() != *SI) {
	  ...
	  errs() << "Forcing " << BE->getName() << " -> " << Header->getName() << " to become a backedge\n";
